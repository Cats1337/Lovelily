...
class EDD_SL_Plugin_Updater {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
					...
					__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'mage-eventpress' ),
				...
				printf(
					 // translators: %s is the event label (e.g., "Event", "Workshop").
					__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'mage-eventpress' ),
					esc_html( $version_info->name ),
					'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $version_info->new_version ),
					'</a>',
	...
	public function show_changelog() {
		...
		if( ! current_user_can( 'update_plugins' ) ) {
			wp_die( __( 'You do not have permission to install plugin updates', 'mage-eventpress' ), __( 'Error', 'mage-eventpress' ), array( 'response' => 403 ) );
		}
